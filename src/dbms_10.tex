...
\textbf{Size (proposal):} about 6 pages \\
...
\begin{infobox}
\textbf{Goal of this exercise:} Submit an approx.\ 6-page project proposal that states
...
Submit a PDF of \textbf{about 6 pages} containing the following sections. Use the
...
\subsection{Frontend prototype (hand sketches)}

Describe what the frontend looks like and which endpoints it operates (L 10), and
document a \textbf{prototypical frontend with hand-drawn sketches}. Provide
\textbf{one sketch per screen/form} (including the startup connection dialog) that
shows the layout, the inputs and which endpoint the screen calls -- a clean hand
drawing, photographed, is entirely sufficient. State whether you use the
recommended Python variant (tkinter) or something else -- and in the latter case,
what and why. Note that the frontend is delivered as a \textbf{\texttt{.deb}
installer} and collects the API URL and key in a \textbf{connection dialog} at
startup.
...
Estimate the scope honestly: how many tables and endpoints? Give \textbf{2--3
milestones} as a small \textbf{schedule} that maps each milestone to a week and
an estimated number of hours (e.g.\ ``schema + seed data ready'', ``API
reading/writing'', ``frontend operates the core endpoints'', ``tests +
documentation + video''); the hours should add up to roughly 40. Name
\textbf{one or two risks} that could make it fail. Keep the total within the
\textbf{$\approx$40\,h / 2-month} budget.

\subsection{Acceptance criteria and testing}

State \emph{when} your system counts as ``done and correct'' and \emph{how} you
prove it. Give a handful of \textbf{acceptance criteria} as checkable statements
about the finished system (e.g.\ ``a record that violates a constraint is
rejected with a 4xx'', ``a write endpoint without a valid \texttt{X-API-Key} is
refused'', ``\texttt{GET /\dots} returns the correct aggregated value'') and a
short \textbf{test plan} that names what you test and with which tools --
database constraints (PK/FK/UNIQUE/NOT NULL), API endpoints (happy path and
error cases, e.g.\ with \texttt{pytest} and FastAPI's \texttt{TestClient}, L 08)
and the business rule behind the risk you named above. Turn that risk into at
least one concrete test case.
...
\begin{itemize}[leftmargin=1.4em]
  \item \textbf{Format:} one PDF, about 6 pages, with the sections from
...
\begin{itemize}[leftmargin=1.8em, label=$\square$]
  ...
  \item A prototypical frontend is documented with a hand sketch per screen/form
        (including the connection dialog), each naming the endpoint it calls; it
        is delivered as a \texttt{.deb}.
  \item The Docker Compose backend (postgres + api, \texttt{.env}) and its
        deployment on the lecture server are named.
  \item Scope, milestones (with a week/hours schedule) and risks are realistic
        and fit the $\approx$40\,h / 2-month budget.
  \item Acceptance criteria and a short test plan are given; the named risk is
        turned into at least one concrete test case.
...
\medskip
\textbf{Milestones.} (1)~week 1: schema + seed data ($\approx$8\,h);
(2)~weeks 2--3: API reading/writing ($\approx$12\,h); (3)~weeks 4--5: frontend
operates both tabs, packaged as \texttt{.deb} ($\approx$10\,h); (4)~week 6:
tests + documentation + video ($\approx$10\,h). \textbf{Risk:} correct return
handling when several open loans of the same book exist at once.

\medskip
\textbf{Acceptance criteria \& tests.} A book with \texttt{stock\,=\,0} cannot be
lent (\texttt{POST /loans} returns 409); \texttt{GET /books} shows the correct
open-loan count per book; \texttt{POST} without a valid \texttt{X-API-Key} is
refused (401). Verified with \texttt{pytest} + FastAPI \texttt{TestClient}; the
return-handling risk gets its own test with two open loans of the same book.
\end{beispielbox}
