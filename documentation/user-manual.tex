...
\renewcommand{\thgaDatum}{Version 0.1.2}
...
{Installation, daily operation and troubleshooting - Version 0.1.2}
...
For version 0.1.2 the filename is:

\begin{center}
\texttt{fittracker-frontend\_0.1.2\_amd64.deb}
\end{center}
...
\texttt{amd64} and version \texttt{0.1.2}.
...
\textbf{Exercise Dictionary}. Version 0.1.2 provides the seeded demo user, so
...
\begin{figure}[H]
  ...
  \caption{The 15 seeded exercises in version 0.1.2.}
...
\begin{itemize}[leftmargin=1.4em]
  ...
  \item the date must use a valid \texttt{YYYY-MM-DD} value; and
...
again. After changing \texttt{.env}, recreate the API container:
...
field contains \texttt{1}. User registration is not part of version 0.1.2.
...
\begin{enumerate}[leftmargin=1.6em]
  ...
  \item A write request with a missing or incorrect API key returns HTTP 401.
\end{enumerate}
...
Version 0.1.2 intentionally implements the approved term-project scope:
...
When reporting a problem, include the application version, operating system,
the output of \texttt{docker compose ps} and the relevant container logs. Never
include the real API key or database password.
